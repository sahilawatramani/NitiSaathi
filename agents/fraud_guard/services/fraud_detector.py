...
_PREDATORY_LENDERS = _load_json_list('rbi_nbfc_list.json', 'predatory_lenders')
_CAUTION_ENTITIES = _load_json_list('rbi_nbfc_list.json', 'caution_entities')
_SCAM_KEYWORDS = _load_json_list('rbi_nbfc_list.json', 'known_scam_keywords')
...
class FraudDetectionEngine:
    ...
    def _check_velocity_anomaly(self, transactions: list[dict]) -> dict | None:
        """Detect ≥3 transactions with same/similar merchant or amount within 10 minutes."""
        if not transactions:
            return None
        # Group by merchant name (case-insensitive)
        by_merchant = defaultdict(list)
        for txn in transactions:
            ts_raw = txn.get('date') or txn.get('txn_date') or txn.get('timestamp')
            if not ts_raw:
                continue
            try:
                ...
                continue
            merchant = (txn.get('merchant') or txn.get('description') or '').lower().strip()
            amount = float(txn.get('amount', 0))
            by_merchant[merchant].append({'ts': ts, 'amount': amount})
        
        for merchant, entries in by_merchant.items():
            if len(entries) < 3:
                continue
            entries_sorted = sorted(entries, key=lambda x: x['ts'])
            for i in range(len(entries_sorted) - 2):
                window_start = entries_sorted[i]['ts']
                window_end = entries_sorted[i + 2]['ts']
                if (window_end - window_start) <= timedelta(minutes=10):
                    return {
                        'alert': True,
                        'alert_message': f'Suspicious: {min(len(entries), 10)} transactions to "{merchant}" detected within 10 minutes. This may indicate unauthorized use.',
                        'risk_score': 0.85,
                        'velocity_merchant': merchant,
                        'velocity_count': len(entries)
                    }
        return None
    ...
    def detect_fraud(
        ...
        Args:
            ...
            recent_transactions: Optional list of recent raw transaction dicts for velocity check
        ...
        if recent_transactions:
            velocity_result = self._check_velocity_anomaly(recent_transactions)
            if velocity_result:
                velocity_alert = FraudAlert(
                    ...
                    matched_patterns=[],
                    alert_title="⚠️ High Velocity Detected",
                    alert_message=velocity_result['alert_message'],
                    user_warning="Multiple transactions to the same merchant in a short time. Please verify.",
                    action_required="Review recent transactions immediately.",
                    contains_pin_otp_request=False,
                    transaction_details=transaction.model_dump() if hasattr(transaction, 'model_dump') else transaction.dict()
                )
    ...
    def check_rbi_lender(self, entity_name: str) -> RBILenderCheckResult:
        ...
        # Check predatory lenders
        for lender in _PREDATORY_LENDERS:
            if lender.lower() in entity_lower:
                return RBILenderCheckResult(
                    entity_name=entity_name,
                    is_registered=False,
                    warning="⚠️ This entity is a known predatory lender. Do not engage with them.",
                    source="RBI Caution List"
                )
                
        # Check caution entities
        for entity in _CAUTION_ENTITIES:
            if entity.lower() in entity_lower:
                return RBILenderCheckResult(
                    entity_name=entity_name,
                    is_registered=False,
                    warning="⚠️ This entity is on a caution list. Be very careful.",
                    source="RBI Caution List"
                )
        ...
        for broker in _REGISTERED_BROKERS:
            if broker.lower() in entity_lower:
                return RBILenderCheckResult(
                    entity_name=entity_name,
                    is_registered=True,
                    entity_type="Broker",
                    source="SEBI Registered List"
                )
