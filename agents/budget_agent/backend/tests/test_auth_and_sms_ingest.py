...
def _signup_and_login_token(client: TestClient):
...
def test_sms_ingest_creates_pending_event(client: TestClient):
...
def test_webhook_rejects_invalid_secret(client: TestClient):
    payload = {
...
def test_sms_forward_requires_secret_config_or_valid_header(client: TestClient):
    payload = {
