...
def _profile_projection(profile: UserProfile | None, user_id: int) -> dict[str, Any]:
    ...
    return {
        ...
        "epfo_esic_status": bool(getattr(profile, "epfo_esic_status", False)) if profile else False,
        "income_tax_payer": bool(getattr(profile, "income_tax_payer", False)) if profile else False,
        "days_active_with_aggregator": int(getattr(profile, "days_active_with_aggregator", 0) or 0) if profile else 0,
        "e_shram_registered": bool(getattr(profile, "e_shram_registered", False)) if profile else False,
        "savings_bank_account": bool(getattr(profile, "savings_bank_account", True)) if profile else True,
        "aadhaar_linked": bool(getattr(profile, "aadhaar_linked", True)) if profile else True,
        "state": getattr(profile, "state", None) if profile else None,
        "language_pref": (getattr(profile, "language_pref", "en") or "en") if profile else "en",
        "literacy_level": (getattr(profile, "literacy_level", "medium") or "medium") if profile else "medium",
