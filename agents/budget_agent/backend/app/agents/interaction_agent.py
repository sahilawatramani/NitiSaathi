...
def handle_user_query(query: str, context: str) -> str:
    ...
    # Step 1: Search knowledge base for relevant info (top 2 concise snippets)
    rag_results = rag_service.search(query, k=2)
    kb_context = "\n".join([res["document"] for res in rag_results]) if rag_results else ""
    
    if not kb_context:
        kb_context = "No specific knowledge base entry found."
...
Instructions:
- Provide actionable, personalized advice for Indian users (INR, 80C/80D, SIPs, emergency fund).
- Keep your response clear and concise (under 150 words, using 3-4 bullet points and bold highlights).
